...
class Card::Identifier
  ...
  # a number which uniquely identifies both which bank the card belongs to,
  # and whether it is a business card or a personal one. this forms part of
  # the unique identifier for each card, which allows the admin to determine
  # these things about the card at a glance.
  #
  # The bank code is determined by the bank's id, which is always an odd
  # number. If this is a personal card, bank_number is equal to bank.id. If
  # this is a business card, bank_number is equal to bank.id + 1.
  #
  ...
  def bank_code
    "%.2d" % (@card.personal? ? @bank.id : @bank.id + 1)
  end
