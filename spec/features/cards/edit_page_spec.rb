...
RSpec.describe 'card accounts edit page', :js do
  ...
  example 'opened card has "closed at" hidden by default' do
    ...
    expect(page).to have_no_selector('#card_closed_at_1i')
    expect(page).to have_no_selector('#card_closed_at_2i')
    check :card_closed
    expect(page).to have_selector('#card_closed_at_1i')
    expect(page).to have_selector('#card_closed_at_2i')
    uncheck :card_closed
    expect(page).to have_no_selector('#card_closed_at_1i')
    expect(page).to have_no_selector('#card_closed_at_2i')
  end
  ...
  example 'valid update' do
    ...
    check :card_closed
    submit_form
    expect(opened_card.reload.closed_at).to be_present
  ...
  example 'unclosing a closed card' do
    ...
    uncheck :card_closed
    submit_form
    expect(closed_card.reload.closed_at).to be nil
    expect(current_path).to eq cards_path
  end
  ...
  example 'invalid update' do
    ...
    check :card_closed
    closed = 5.years.ago
    raise unless closed < opened_card.opened_at # make sure it will actually fail
    select closed.year, from: :card_closed_at_1i
    select Date::MONTHNAMES[closed.month][0..2], from: :card_closed_at_2i
    submit_form
    expect(opened_card.reload.closed_at).not_to be_present
