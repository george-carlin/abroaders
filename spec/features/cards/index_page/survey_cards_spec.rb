...
describe "card accounts page survey cards section" do
  ...
  example "no companion; I have 'from survey' cards" do
    ...
    open_acc_on_page   = CardOnPage.new(open_acc, self)
    closed_acc_on_page = CardOnPage.new(closed_acc, self)
  ...
  example "companion; both people have survey cards" do
    m_open   = create(:open_survey_card,   person: person, product: products[0])
    m_closed = create(:closed_survey_card, person: person, product: products[1])
    create_companion!
    p_open   = create(:open_survey_card,   person: companion, product: products[0])
    p_closed = create(:closed_survey_card, person: companion, product: products[1])

    m_open   = CardOnPage.new(m_open,   self)
    m_closed = CardOnPage.new(m_closed, self)
    p_open   = CardOnPage.new(p_open,   self)
    p_closed = CardOnPage.new(p_closed, self)
