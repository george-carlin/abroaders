...
describe "as a user viewing my cards" do
  ...
  shared_examples "card recommendations" do
    context "when I have been recommended some cards" do
      let(:extra_setup) do
        @recs = create_list(:card_recommendation, 2, person: person)
      end
      ...
      describe "clicking the 'decline' button", :js do
        ...
        it "shows a text field asking why I'm declining" do
      ...
      context "when I have clicked 'Apply' on a recommendation" do
        let(:extra_setup) do
          @rec = create(:clicked_card_recommendation, person: person)
        end
        let(:rec)   { @rec }
        let(:offer) { rec.offer }
        ...
        it "asks whether I applied etc"
      end
    end
  end

  context "for the main person" do
    let(:person) { me }
    let(:selector) { main_recommendations_selector }
    include_examples "card recommendations"
  end

  context "when I have a partner" do
    let(:has_partner) { true }
    let(:person) { partner }
    let(:selector) { partner_recommendations_selector }
